...
uses
  ...
  StdCtrls, Db, DBTables, Math, Mask, RXCtrls, DateUtils;
...
type
  TPrintActForm = class(TForm)
    ...
    qMTRQuery: TQuery;
    NDSQuery: TQuery;
    DocNumEdit: TMaskEdit;
    RxSpeedButton1: TRxSpeedButton;
    procedure CreateReport(iddoc: integer);
    function conv2words (number: integer; level: integer; mony: integer) : string;
    function float2words(number: real; mony: integer): string;
...
begin
   ...
   with qMTRQuery do
    ...
    StrDate := StrMon + ' ' + inttostr(Year)+' г.';
...
//                                                ' '+GetYearStr(qMainQuery.FieldByName('date_in').AsDateTime), 0);
        ...
        replacePattern(Form.Editor, 'NumDoc' ,  qMainQuery.FieldByName('NumDoc').asstring, 1);
        replacePattern(Form.Editor, 'TodayDate' , DateToStr(Date), 0);
        replacePattern(Form.Editor, 'DateBeg' , StrDate, 1);
//        replacePattern(Form.Editor, 'DateBeg' , DateToStr(EncodeDate(YearOf(Date), MonthOf(Date), 1)), 0);
//        replacePattern(Form.Editor, 'DateEnd' , DateToStr(EncodeDate(YearOf(Date), MonthOf(Date), DaysInAMonth(YearOf(Date), MonthOf(Date)))), 0);
       ...
       y := Form.Editor.caretpos.y;
        while not qTableQuery.eof do
          begin
             ...
             replacePattern(Form.Editor, 'NMNTName',  qTableQuery.FieldByName('NMNT_Name').asstring, 0);
             ...
             y := y + 1;
             lines.insert(y,tablestr);
             replacePattern(Form.Editor, 'EQPTName',  qTableQuery.FieldByName('CHRT_Name').asstring, 0);
...
//             replacePattern(Form.Editor, 'EQPTName',  qTableQuery.FieldByName('Name_1').asstring, 0);
             replacePattern(Form.Editor, 'CHRTCode',  qTableQuery.FieldByName('CHRT_Code').asstring, 0);
             replacePattern(Form.Editor, 'PriceEd',   FormatFloat('0.000', qTableQuery.FieldByName('Ntime').AsFloat/1000) , 2);
             replacePattern(Form.Editor, 'Ediz',      qTableQuery.FieldByName('Ediz_code').asstring, 1);
...
//          replacePattern(Form.Editor, 'TotalSum', FormatFloat( '0.00',  TotalSum), 2);
 ...
 //     символьная запись TotalTot
          stringvalue:=float2words(TotalTot, 0);
...
//numebr - число переводимое в строку
//mony - логическая переменная 0 - рубли 1 - число
function TPrintActForm.float2words(number: real; mony: integer): string;
...
begin
          ...
          begin
             ...
             digitcount:=trunc(log10(Trunc(number)))+1;
                ...
                begin
                 {вызываем функцию перевода чисел в слова, в качестве числа для
                  ...
                  миллионы - тысячи - сотни) }
                   stringvalue:=conv2words(Trunc(Frac(number/power(1000,counter))*1000),counter,mony)+stringvalue;
             ...
             If mony = 0 Then
                 stringvalue:=conv2words(Trunc(Frac(number/power(1000,counter))*1000),counter,mony)
                           +stringvalue+FloatToStr(Round(Frac(number)*100))+' коп.'
              else
              begin
                    tmp := Frac(number/power(1000,counter))*1000;
                    stringvalue:=conv2words(Trunc(tmp),counter,mony)+stringvalue;
                    If Round(Frac(number)*100) > 0 Then stringvalue:=stringvalue+FloatToStr(Round(Frac(number)*100));
              end;
          end
          else
             If mony = 0 Then
               {если введенное число - ноль, то пишем "ноль" :) }
                stringvalue:='ноль рублей ' + FloatToStr(Frac(number))+' коп.'
              else
                   stringvalue:='ноль';
...
{ converting function
   ...
   $mony - логическая переменная 0 - рубли 1 - число}
function TPrintActForm.conv2words (number: integer; level: integer; mony: integer): string;
...
procedure TPrintActForm.RxSpeedButton1Click(Sender: TObject);
begin
  if docnumedit.Text <> '' then
    CreateReport(strtoint(trim(docnumedit.Text)));
end;

procedure TPrintActForm.Button2Click(Sender: TObject);
begin
  close;
end;
