...
procedure TJR_PREQPTForm.CloseButtonClick(Sender: TObject);
begin
  close;
end;
{
...
procedure TJR_PREQPTForm.ButtonAddClick(Sender: TObject);
var DocForm: TDOC_PREQPTForm;
begin
  if not check_user_grand('DH_PR_EQPT', 'I', true) then exit;

  // открыть форму редактирования нового документа
  DocForm := TDOC_PREQPTForm.CreateN(mainform, - 1);
  DocForm.show;
...
procedure TJR_PREQPTForm.ButtonEditClick(Sender: TObject);
var DocForm: TDOC_PREQPTForm;
begin
  if  (data.JRPREQPTFormQuery.FieldByName('ID').isnull) then exit;

  IDDoc := data.JRPREQPTFormQuery.FieldByName('ID').asinteger;
...
procedure TJR_PREQPTForm.ButtonDelClick(Sender: TObject);
var number: string;
    date  : string;
begin
  ...
  Number := data.JRPREQPTFormQuery.FieldByName('NumDoc').AsString;
  date   := data.JRPREQPTFormQuery.FieldByName('Date_in').AsString;

  if messagedlg('Удалить поступление инструмента  ' + Number + ' от ' + date + '?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
   begin
    IDDoc := data.JRPREQPTFormQuery.FieldByName('ID').AsInteger;

    if not data.JRPREQPTFormQuery.Eof then
      data.JRPREQPTFormQuery.Prior;

    Stored_Del_Proc.ParamByName('IDDOC').asinteger := IDDoc;  //  записи.
    Stored_Del_Proc.ExecProc;
   end;

   refreshquery(data.JRPREQPTFormQuery);
...
procedure TJR_PREQPTForm.ButtonIntClick(Sender: TObject);
begin
  ...
  with data.JRPREQPTFormQuery do
    begin
...
procedure TJR_PREQPTForm.FormShow(Sender: TObject);
...
begin
  ...
  with data.JRPREQPTFormQuery do
    ...
    begin
       sql.clear;
       sql.add('select * from BROWSE_PREQPTDOC_PROC(:dateIn, :DateOut, :ID_CST_FILTER, :ID_WRHS_Filter)');
       ParamByName('DateIn').asdate  := FromDate;
       ParamByName('DateOut').asdate := ToDate;
       ParamByName('ID_CST_FILTER').asinteger := ID_CST_Filter;
       ParamByName('ID_WRHS_FILTER').asinteger:= ID_WRHS_Filter;
  ...
  if Param_Form.glOpenAllFormMaximized then
   ...
   filterstring := GetColumnsList('BROWSE_PREQPTDOC_PROC', FormGrid);
...
procedure TJR_PREQPTForm.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  SetColumnList('BROWSE_PREQPTDOC_PROC', FormGrid, data.JRPREQPTFormQuery, filterstring);
  action := caFree;
end;
...
procedure TJR_PREQPTForm.ToolButtonFindDownClick(Sender: TObject);
begin

 if Searchkey.text = '' then exit;
   if not data.JRPREQPTFormQuery.eof then data.JRPREQPTFormQuery.next;

    find(data.JRPREQPTFormQuery, FormGrid, Searchkey.text, 1);
...
procedure TJR_PREQPTForm.ToolButtonFindUpClick(Sender: TObject);
begin

 if Searchkey.text = '' then exit;

   if not data.JRPREQPTFormQuery.bof then data.JRPREQPTFormQuery.prior;
...
procedure TJR_PREQPTForm.FindButtonClick(Sender: TObject);
begin
...
procedure TJR_PREQPTForm.SearchkeyKeyPress(Sender: TObject; var Key: Char);
begin
  if key = #13 then
    find(data.JRPREQPTFormQuery, FormGrid, Searchkey.text, 1);
...
procedure TJR_PREQPTForm.FormGridKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
 ...
 IdDoc  := data.JRPREQPTFormQuery.FieldByName('ID').asinteger;
  ...
  case Key of
    ...
    VK_F5 : refreshquery( data.JRPREQPTFormQuery); // обновление таблицы
  end;

end;

procedure TJR_PREQPTForm.ToolButtonHelpClick(Sender: TObject);
begin
...
procedure TJR_PREQPTForm.MenuAddAnotherClick(Sender: TObject);
begin
end;
{
...
procedure TJR_PREQPTForm.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TJR_PREQPTForm.MenuRefreshClick(Sender: TObject);
begin
...
procedure TJR_PREQPTForm.sort_table(type_of_sort: integer);
var strord : string;
begin
...
procedure TJR_PREQPTForm.MenuSortByDateClick(Sender: TObject);
begin
  sort_table(1);
end;
...
procedure TJR_PREQPTForm.MenuSortByDate1Click(Sender: TObject);
begin
  sort_table(2);
end;
...
procedure TJR_PREQPTForm.MenuByWidthClick(Sender: TObject);
begin
...
procedure TJR_PREQPTForm.ButtonFilterClick(Sender: TObject);
...
procedure TJR_PREQPTForm.FormGridTitleBtnClick(Sender: TObject;
...
procedure TJR_PREQPTForm.ColumnSet(Sender: TObject);
var item : tMenuItem;
    i : integer;
begin
