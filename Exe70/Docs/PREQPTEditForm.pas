...
var
  DOC_PREQPTForm: TDOC_PREQPTForm;
  IDDoc         : integer;  //  идентификатор документа
  NumDoc        : Int64;  //  номер документа
...
procedure  TDOC_PREQPTForm.DocumentChange;
begin
 WasAnyChanges := true;
 caption := DocCaption + ' *';
end;
...
function TDOC_PREQPTForm.SaveDoc: boolean;
var _s : string;
     i : integer;                 //
...
begin
  ...
  result := true;
 if not check_user_grand('DH_PR_EQPT', 'U', true) then exit;
      ...
      begin
       ...
       begin
          ...
          begin
               ...
               if (ChechUniqueCode('DH_PR_EQPT', 'NUMDOC', _s, IDDoc) > 0)  then
               begin
                  result :=  false;
...
{    If Trim(DocNum.text) <> 'Новый' Then
    begin  //Если код документа введен в ручную
           ...
           LogicQuery.SQL.Add('Select * From DH_PR_Eqpt Where numdoc = :code');
           LogicQuery.ParamByName('code').AsFloat := StrToInt64(Trim(DocNum.text));
           LogicQuery.Open;
    ...
    // перебор всех строк плановых операций документа
     ...
     for i := 0 to MemoryData.RecordCount - 1 do
      ...
      begin
        ...
        StorProcTable.ParamByName('ID_PREQPT').asinteger:= IDDoc;
        ...
        if StorProcTable.ParamByName('ERROR').asinteger = 1 then
          begin
            MessageDlg('На дату ' + DateDoc.text + ' у человека ' + WRKRCombo.Text
...
constructor TDOC_PREQPTForm.CreateN(AOwner: TForm; docID : integer);
begin
  ...
  if IDDoc = - 1 then
    ...
    begin // для нового документа
      ...
      DocCaption := 'Документ Поступление инструмента: Новый';
      WasAnyChanges := true;
    end
    ...
    begin
      ...
      qFormQuery.sql.add('select * from dh_pr_eqpt where id = ' + inttostr(IDDoc));
      qFormQuery.open;
      ...
      DocCaption := 'Документ Поступление инструмента: ' + DocNum.text + ' от ' + DateDoc.text;
...
destructor TDOC_PREQPTForm.Destroy;
begin
  inherited;
end;
{
...
procedure TDOC_PREQPTForm.OKButtonClick(Sender: TObject);
begin
...
procedure TDOC_PREQPTForm.CloseClick(Sender: TObject);
begin
...
procedure TDOC_PREQPTForm.FormClose(Sender: TObject;
...
procedure TDOC_PREQPTForm.WRHSComboButtonClick(Sender: TObject);
begin
  DocumentChange;
...
procedure TDOC_PREQPTForm.CSTComboButtonClick(Sender: TObject);
begin
  DocumentChange;
...
procedure TDOC_PREQPTForm.CSTComboKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  DocumentChange;
...
procedure TDOC_PREQPTForm.WRHSComboKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  DocumentChange;
...
procedure TDOC_PREQPTForm.ToolButtonAddClick(Sender: TObject);
var idediz : pRecord;
begin
  DocumentChange;
...
procedure TDOC_PREQPTForm.ToolButtonEditClick(Sender: TObject);
begin
...
procedure TDOC_PREQPTForm.ToolButtonDelClick(Sender: TObject);
var i : integer;
begin
  if MemoryData.RecordCount = 0 then exit;
  MemoryData.Delete;
...
procedure TDOC_PREQPTForm.ToolButtonUpClick(Sender: TObject);
var i,j : integer;
begin
...
procedure TDOC_PREQPTForm.ToolButtonDownClick(Sender: TObject);
var i,j : integer;
begin
...
procedure TDOC_PREQPTForm.GridTableKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  case Key of
...
procedure TDOC_PREQPTForm.DateDocChange(Sender: TObject);
begin
...
procedure TDOC_PREQPTForm.PrintButtonClick(Sender: TObject);
begin
...
procedure TDOC_PREQPTForm.NeedBillButtonClick(Sender: TObject);
begin
   if iddoc < 0 then exit;
...
procedure TDOC_PREQPTForm.WRKRComboButtonClick(Sender: TObject);
begin
  DocumentChange;
...
procedure TDOC_PREQPTForm.WRKRComboKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  DocumentChange;
...
procedure TDOC_PREQPTForm.Type_docClick(Sender: TObject);
begin
...
procedure TDOC_PREQPTForm.DBGridEh1Columns1EditButtonClick(Sender: TObject;
  var Handled: Boolean);
var idediz : precord;
begin
  DocumentChange;
...
procedure TDOC_PREQPTForm.DBGridEh1Columns2EditButtonClick(Sender: TObject;
  var Handled: Boolean);
begin
  DocumentChange;
  MemoryData.Edit;
...
procedure TDOC_PREQPTForm.GridKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  case Key of
    46 :  ToolButtonDelClick(Sender);   // удаление записи
