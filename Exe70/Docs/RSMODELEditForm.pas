...
var
  DOC_RSMODELForm: TDOC_RSMODELForm;
...
procedure  TDOC_RSMODELForm.DocumentChange;
begin
 WasAnyChanges := true;
 caption := DocCaption + ' *';
end;
...
function TDOC_RSMODELForm.SaveDoc: boolean;
var _s : string;
     i : integer;                 //
...
begin
  ...
  result := true;
 if not check_user_grand('DH_RS_MODEL', 'U', true) then exit;
  ...
  try
    ...
    if not NewDoc then
      if (ChechUniqueCode('DH_RS_MODEL', 'NUMDOC', _s, IDDoc) > 0)  then
    ...
    if (CSTCombo.ID > 0) and (CstCombo.Visible) then
      ...
      begin
        showmessage('Не проставлен получатель...');
        result :=  false;
...
constructor TDOC_RSMODELForm.CreateN(AOwner: TForm; docID : integer);
...
begin
  ...
  if IDDoc = - 1 then
    ...
    begin // для нового документа
      ...
      DocCaption := 'Документ Списание моделей: Новый';
      WasAnyChanges := true;
    end
    ...
    begin
      ...
      qFormQuery.sql.add('select * from dh_rs_model where id = ' + inttostr(IDDoc));
      qFormQuery.open;
      ...
      qTableQuery.open;
      DocCaption := 'Документ Списание моделей: ' + DocNum.text + ' от ' + DateDoc.text;
...
destructor TDOC_RSMODELForm.Destroy;
begin
  inherited;
end;
{
...
procedure TDOC_RSMODELForm.OKButtonClick(Sender: TObject);
begin
...
procedure TDOC_RSMODELForm.CloseClick(Sender: TObject);
begin
...
procedure TDOC_RSMODELForm.FormClose(Sender: TObject;
...
procedure TDOC_RSMODELForm.WRHSComboButtonClick(Sender: TObject);
begin
  DocumentChange;
...
procedure TDOC_RSMODELForm.CSTComboButtonClick(Sender: TObject);
begin
  DocumentChange;
...
procedure TDOC_RSMODELForm.CSTComboKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  DocumentChange;
...
procedure TDOC_RSMODELForm.WRHSComboKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  DocumentChange;
...
procedure  TDOC_RSMODELForm.SortTable;
begin
...
procedure TDOC_RSMODELForm.FormShow(Sender: TObject);
begin
...
procedure TDOC_RSMODELForm.ToolButtonAddClick(Sender: TObject);
begin
  DocumentChange;
  MemoryData.Insert;
...
procedure TDOC_RSMODELForm.ToolButtonEditClick(Sender: TObject);
begin
...
procedure TDOC_RSMODELForm.ToolButtonDelClick(Sender: TObject);
var i : integer;
begin
  if MemoryData.RecordCount = 0 then exit;
...
procedure TDOC_RSMODELForm.ToolButtonUpClick(Sender: TObject);
var i,j : integer;
begin
...
procedure TDOC_RSMODELForm.ToolButtonDownClick(Sender: TObject);
var i,j : integer;
begin
...
procedure TDOC_RSMODELForm.GridTableKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
//  case Key of
...
procedure TDOC_RSMODELForm.DateDocChange(Sender: TObject);
begin
...
procedure TDOC_RSMODELForm.PrintButtonClick(Sender: TObject);
begin
...
procedure TDOC_RSMODELForm.NeedBillButtonClick(Sender: TObject);
begin
   if iddoc < 0 then exit;
...
procedure TDOC_RSMODELForm.GridColumns1EditButtonClick(Sender: TObject;
  var Handled: Boolean);
begin
  DocumentChange;
...
procedure TDOC_RSMODELForm.GridKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  case Key of
    46 :  ToolButtonDelClick(Sender);   // удаление записи
